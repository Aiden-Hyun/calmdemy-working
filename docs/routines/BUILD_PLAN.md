...
**M4 — Green Light** — ✅ done 2026-07-01 (`tsc` + `eslint` clean)
- [x] `domain/greenLight.ts` (`computeGreenLight` priority-weighted; `done`/`rest`/`shielded` satisfy; thresholds 0.8/0.4 as tunable constants — OQ3).
...
**M6 — Standalone data islands** — ✅ done 2026-07-01 (`tsc` + `eslint` clean)
- [x] Feature 8: `api/todos.ts` (CRUD + `getTodosForRange`), `hooks/useTodos.ts`, `components/TodoRow.tsx`, `screens/TodosScreen.tsx` + route (quick add Today/Tomorrow/Someday, check-off, delete). **Surfaced as its own "To-dos" tile on the Tools tab** (`todosManifest`), not via the Routines header.
...
- [x] Feature 10: `api/trackers.ts` (defs + date-keyed `entries` subcollection), `hooks/useTrackers.ts`, `data/trackerKinds.ts` (+`formatTrackerValue`), `TrackerChart` (react-native-chart-kit LineChart), `TrackersScreen` + `TrackerDetailScreen` + routes. **Surfaced as its own "Trackers" tile on the Tools tab** (`trackersManifest`).
- [x] Feature 13: `hooks/useCountdownTimer.ts` (counts into overtime), `components/RoutineTimer.tsx` (presets, red overtime, play/pause/reset), `TimerScreen` + `app/routines/timer.tsx`; launched from `HabitDetailScreen` ("Start focus timer", passes the habit name as `label`).
...
- **OQ3 — [IMPLEMENTED, confirm later]:** Green Light thresholds shipped as `GREEN_THRESHOLD = 0.8` / `YELLOW_THRESHOLD = 0.4` constants in `domain/greenLight.ts`. Tune before public release if design wants different cutoffs (one-line change).
...
| 2026-07-01 | **M6 pass 2 implemented & verified** (`tsc --noEmit` + `eslint` clean). Features 10/13: trackers (`api/trackers.ts` + `entries` subcollection, `hooks/useTrackers.ts`, `TrackerChart` via react-native-chart-kit, `TrackersScreen`/`TrackerDetailScreen`) and focus timer (`useCountdownTimer`, `RoutineTimer`, `TimerScreen`). Query keys `["trackers", uid]` / `["trackerEntries", uid, id, max]`. `routineTrackers` (+ nested `entries`) rule shipped in M0; tracker-entry chart query is a single-field `dateKey` sort (auto index). Data model unchanged. |
| 2026-07-01 | **Restructure + bug fix** (`tsc`/`eslint` clean). Added `todosManifest` + `trackersManifest` (routines module exports two extra manifests — kept in-module to avoid `boundaries` violations; routes stay under `/routines/*`) so To-dos & Trackers are their own Tools tiles; removed their buttons from the Routines header. Fixed `TodoRow` invisible-title bug (flex-through-AnimatedPressable collapse) by rebuilding it as a full-width View + `flex:1` text `Pressable`. **Convention note:** a feature module may export additional `*Manifest`s for sub-features that warrant their own Tools tile. |
| 2026-07-01 | **Optimistic check-off** (`tsc`/`eslint` clean). `useToggleTodo` and `useToggleCompletion` now use React Query `onMutate` to patch the cache immediately (todo checkbox flips instantly; habit check + Green Light flip instantly), with `onError` rollback and `onSettled` reconciling invalidation — removing the write→refetch round-trip lag. **Pattern for future mutations:** optimistic `onMutate`/`onError`/`onSettled` for any tap-to-toggle interaction. |
| 2026-07-01 | **All mutations optimistic** (`tsc`/`eslint` clean). Added `hooks/optimistic.ts` (`optimisticList` helper: snapshot → apply → rollback → settle for single list caches). Made optimistic: todo create/delete (flat + month caches, inline); tracker create/delete + log-value (entry upsert); profile create/delete/**apply** (active swap is instant); goal-tag create; habit create/**update** (list + detail, inline)/**archive**; and `useSpendShield`. Optimistic creates use a `temp-…` id the reconciling refetch replaces. **Convention:** every user-facing add/remove/toggle in the suite is now optimistic — keep new mutations that way (`optimisticList` for single-list cases; inline for multi-cache). |
...
| 2026-07-01 | **Committed + pushed M6 pass 1** (`8c374f1`) to `origin`. | 8, 9 | M6 pass 2 |
| 2026-07-01 | **M6 pass 2 complete (features 10, 13) — M6 DONE.** `api/trackers.ts` + `hooks/useTrackers.ts` + `data/trackerKinds.ts` + `TrackerChart` (chart-kit); `TrackersScreen` + `TrackerDetailScreen` + routes; trackers button on Today. `useCountdownTimer` + `RoutineTimer` + `TimerScreen` + route; launched from HabitDetailScreen. `tsc` + `eslint` clean. **Not yet committed.** | 10, 13 | M7 — mood & diary (reuse). Also: commit/push pass 2; simulator smoke test still pending. |
| 2026-07-01 | **User feedback: restructure + bug fix (uncommitted, on top of M6 pass 2).** (1) To-dos & Trackers now surface as their own Tools tiles (`todosManifest`/`trackersManifest` in registry + `TOOL_IDS`; removed the to-dos/trackers buttons from the Routines Today header). Routes stay `/routines/todos` + `/routines/trackers`. (2) Fixed `TodoRow`: title text was invisible because `AnimatedPressable` puts `flex` on its inner view (outer Pressable stays content-sized), collapsing the title inside a row — rebuilt as a full-width View with a `flex:1` text `Pressable`. `tsc` + `eslint` clean. | 8, 10 (surfacing) + TodoRow fix | M7 — mood & diary. Also: commit/push M6 pass 2 + this fix; simulator smoke test still pending. |
